...
hawker30_grid <- do.call(rbind,list(grid7 %>% 
  filter(!ID %in% hawker30_grid$ID) %>% 
...
grid5 <- gh_covering(sg_map, precision = 5, minimal = TRUE)
```
...
The next step involves taking the geohashes with isochrone intersections and combining them back with the remaining geohashes that did not have any intersections. 
...
Mapdeck is my favourite R package for making interactive web visualizations. Created by [David Cooley](https://twitter.com/symbolixAU), it uses Uber's open-source [deck.gl](https://deck.gl/) WebGL Javascript library for rendering spatial data alongside Mapbox's tiling service. 

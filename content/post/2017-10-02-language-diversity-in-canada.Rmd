...
van_ldi <- ldi_calc(van_langs_ns) %>%
  mutate(`Region Name` = as.factor(gsub(" \\(.*\\)","",`Region Name`))) 
van_ldi <- inner_join(langs_van, van_ldi)
```
...
# For toronto, we have to split up into two calls as the original clal sometimes bottlenecks the server, pulling down 1.7 million spatial data fields.
langs_tor <- get_census(dataset, level = "DA", regions = regions_list_tor , vectors = language_vectors[1:110], geo_format = "sf", labels = "short")
langs_tor2 <- get_census(dataset, level = "DA", regions = regions_list_tor , vectors = language_vectors[111:219], geo_format = NA, labels = "short")
langs_tor <- inner_join(langs_tor, langs_tor2)

langs_tor_ns <- langs_tor
st_geometry(langs_tor_ns) <- NULL
tor_ldi <- ldi_calc(langs_tor_ns) %>%
  mutate(`Region Name` = as.factor(gsub(" \\(.*\\)","",`Region Name`)))
tor_ldi <- inner_join(langs_tor, tor_ldi)

langs_mtl_ns <- langs_mtl
st_geometry(langs_mtl_ns) <- NULL
mtl_ldi <- ldi_calc(langs_mtl_ns) %>%
  mutate(`Region Name` = as.factor(gsub(" \\(.*\\)","",`Region Name`)))
mtl_ldi <- inner_join(langs_mtl, mtl_ldi)

langs_ott_ns <- langs_ott
st_geometry(langs_ott_ns) <- NULL
ott_ldi <- ldi_calc(langs_ott_ns) %>%
  mutate(`Region Name` = as.factor(gsub(" \\(.*\\)","",`Region Name`))) 
ott_ldi <- inner_join(langs_ott, ott_ldi)
```
...
mv1 <- ggplot(van_ldi) + geom_sf(aes(fill = ldi), colour = NA) + scale_fill_viridis_c("Language Diversity Index",option = "magma") +
  map_theme  +  coord_sf(datum = NA, crs = ) +
